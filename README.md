# Movies Data API
This assessment is based on a small web service, which is implemented using Java, Spring-boot, Hibernate and related technology stack.
Assessment has all required compoments i.e Service layer, Persistence layer, Controller layer, Domain object. For search & filter specifications based REST query language implemented.

Liquibase is used for designing and creating the required database components i.e tables, relations, keys.
Test driven approch is used while developing the application, test cases for all layers are in test package.
